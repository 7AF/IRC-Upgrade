    ...
    public partial class Login : System.Web.UI.Page
    {
        ...
        protected void registerB_Click(object sender, EventArgs e)
        {
            if (username.Text == "" || name.Text == "" || surname.Text == "" || password.Text == "")
            {
                ...
                MessageBox.Show(
                                            "Complete all fields in section Registracija.",
                                            "Error",
            ...
            else
            {
                using (SqlConnection connection = new SqlConnection(connectionStrings))
                {
                    ...
                    using (SqlCommand checkUsername = new SqlCommand("SELECT COUNT(*) FROM Uporabnik WHERE username='" + username.Text + "'", connection))
                    {
                        ...
                        if(count == 0)
                        {
                            using (SqlCommand insertCommand = new SqlCommand("INSERT INTO Uporabnik (username, ime, priimek, geslo) VALUES(@username, @name, @surname, @password)"))
                            {
                        ...
                        {
                            MessageBox.Show(
                                            "Username already exist. Type another one.",
                                            "Error",
                                            MessageBoxButtons.OK,
                                            MessageBoxIcon.Warning,
                                            MessageBoxDefaultButton.Button1,
        ...
        protected void logInB_Click(object sender, EventArgs e)
        {
            if (username1.Text == "" || password1.Text == "")
            {
                ...
                MessageBox.Show(
                                "Complete all fields in section Prijava.",
                                "Error",
                                MessageBoxButtons.OK,
                                MessageBoxIcon.Warning,
                                MessageBoxDefaultButton.Button1,
